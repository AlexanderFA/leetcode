    ...
    function romanToInt(string $s): int
    {
        ...
        for ($i = strlen($s) - 1; $i >= 0; $i--) {
            $counter += match ($s[$i]) {
                'I' => ('V' === $prev || 'X' === $prev) ? -1 : 1,
                'V' => 5,
                'X' => ('L' === $prev || 'C' === $prev) ? -10 : 10,
                'L' => 50,
                'C' => ('D' === $prev || 'M' === $prev) ? -100 : 100,
                'D' => 500,
                'M' => 1000,
            };
...
Assert::eq((new Solution())->romanToInt('MCMXCIV'), 1994);
Assert::eq((new Solution())->romanToInt('LVIII'), 58);
Assert::eq((new Solution())->romanToInt('III'), 3);
